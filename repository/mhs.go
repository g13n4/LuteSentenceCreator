...
func (mhsr *mhsRepository) GetSentences(ctx context.Context, mshq *mhs.QueryHelper, permuts, limit int) ([]string, error) {
	var sChan <-chan *[]int
	var err error
	...
	return mhsr.collectSentenceSet(ctx, sentenceIds, limit)
}
...
func (mhsr *mhsRepository) collectSentenceSet(ctx context.Context, sentenceIdChan <-chan *string, limit int) ([]string, error) {
	...
	query := "SELECT sentence from sentences WHERE id = ANY($1) LIMIT $2"
	rows, err := mhsr.db.Query(ctx, query, ids, limit)
